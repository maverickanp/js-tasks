...
class App {
  async loadCsv(csvFile, separator = ";") {
  ...
  _financing({amount_due, monthlyPay, interest}) {
    let month = 1
  ...
  financing(amount_due, monthlyPay, interest) {    
    const formatter = new Intl.NumberFormat('pt-BR', {
      style: 'currency',
      currency: 'BRL',
    });
    const results = this._financing({
      amount_due, 
      monthlyPay, 
...
const app = new App()
const csv = await app.loadCsv("./src/data/data.csv", ";")
console.log(csv.data)
console.log(csv.filterByCity("Blumenau"))
console.log(csv.sortByfield("cidade", "ASC"))
console.log(app.randomNumbers())
console.log(app.financing(20000, 1000, 2))
